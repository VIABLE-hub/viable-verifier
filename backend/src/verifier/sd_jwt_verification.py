...
def verify_sd_jwt_presentation(raw_token):
    ...
    try:
        ...
        current_nonce = get_nonce_val()
        if not current_nonce:
             # Should check database for valid nonces
             logger.warning("No active nonce found in memory, checking database...")
        ...
        return True, "SD-JWT verified successfully", verified_payload
